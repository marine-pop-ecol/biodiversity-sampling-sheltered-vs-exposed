...
shannon_evenness <- quad_richness %>% 
  mutate(pi_even = 1/richness) %>% 
  mutate(pi_log_even = log(pi_even)) %>% 
  mutate(pi_times_log_pi_even = -(pi_even * pi_log_even)) %>% 
  mutate(pi_tot = richness * pi_times_log_pi_even) %>% 
  select(-c(pi_even, pi_log_even)) %>% 
  cbind(shannon_diversity$pi_times_pi_log)
